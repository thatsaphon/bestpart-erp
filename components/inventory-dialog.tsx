    ...
    <Dialog>
      <DialogTrigger asChild>
        {isButton ? (
          ...
          <a className="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-accent">
            {label}
          </a>
        )}
      </DialogTrigger>
          ...
          <div className="flex justify-center">
            <Tabs defaultValue="account" className="w-[400px]">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="account">รายละเอียดสินค้า</TabsTrigger>
                <TabsTrigger asChild value="price" disabled={!inventory}>
                  <span
                    className={cn(!!inventory && "hover:cursor-pointer")}
                ...
                <TabsTrigger asChild value="picture" disabled={!inventory}>
                  <span
                    className={cn(!!inventory && "hover:cursor-pointer")}
                    data-tooltip-id="tab-disabled-tooltip"
                    data-tooltip-content="จำเป็นต้องสร้างสินค้าก่อน"
              ...
              <TabsContent value="account">
                <div className="p-4 grid grid-cols-[1fr_3fr] content-start gap-3 items-center">
                  <Label className="text-right col-start-1" htmlFor="">
                    รหัสสินค้า
                  </Label>
                  {mode === "create" && (
                    <Input
                  ...
                  {mode === "edit" && (
                    <span className="px-2 ml-2">{inventory?.code}</span>
                  )}
                  <Label className="text-right col-start-1" htmlFor="">
                    ชื่อสินค้า
                  </Label>
                  ...
                  <div className="col-start-2 flex flex-wrap gap-1">
                    {tags.map((tag, i) => (
                      <Fragment key={i}>
                        ...
                        <input hidden value={tag} name="tags" />
                      </Fragment>
                    ))}
                  </div>
                </div>
